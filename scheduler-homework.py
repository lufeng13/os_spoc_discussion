    ...
    if options.policy == 'stride':
        ...
        while jobcount > 0:
            ...
            for i in range(1, jobcount):
                if Stride[i] < Stride[next]:
                    next = i;

            job = runlist.pop(next)
            stride = Stride.pop(next)
            step = Pass.pop(next)
            jobnum  = job[0]
            runtime = float(job[1])
            if response[jobnum] == -1:
            ...
            if runtime > quantum:
                ...
                runtime -= quantum
                stride += step
                print '  [ time %3d ] Run job %3d for %.2f secs | stride from %.2f to %.2f' % (thetime, jobnum, ranfor, stride - step, stride)
                runlist.append([jobnum, runtime])
                ...
                ranfor = runtime
                stride += step
                print '  [ time %3d ] Run job %3d for %.2f secs ( DONE at %.2f ) | stride from %.2f to %.2f' % (thetime, jobnum, ranfor, thetime + ranfor, stride - step, stride)
                turnaround[jobnum] = thetime + ranfor
                jobcount -= 1
            thetime += ranfor
            lastran[jobnum] = thetime
