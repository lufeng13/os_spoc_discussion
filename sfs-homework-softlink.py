...
class fs:
    ...
    # YOUR CODE, 2013011332
        ...
        if self.data[pinode.getAddr()].getFreeEntries() == 0:
            return -1

        # if the newfile was already in parent dir?
        if self.data[pinode.getAddr()].dirEntryExists(newfile):
            return -1
        ...
        self.data[pinode.getAddr()].addDirEntry(newfile, tinum)
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        pinode.incRefCnt()
        self.data[pinode.getAddr()].addDirEntry(newfile, inum)
